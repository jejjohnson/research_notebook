...
def uncertain_variance_numba(
    xtrain, xtest, K, Kinv, weights, mu, signal_variance, length_scale, x_cov
):
    ...
    exp_scale = np.power(length_scale, 2) + 0.5 * np.power(length_scale, 4) * np.power(
        x_cov, -1
    )
    ...
    for itest in range(n_test):
        qi = calculate_q_numba(
            xtrain, xtest[itest, :], K[:, itest], det_term, exp_scale
        )
...
def ard_derivative_numba(x_train, x_function, K, weights, length_scale):
    ...
    for itest in prange(n_test):
        derivative[itest, :] = np.dot(
            np.dot(length_scale, (x_function[itest, :] - x_train).T),
...
def ard_weighted_covariance(
    X, Y=None, x_cov=None, length_scale=None, signal_variance=None
):
