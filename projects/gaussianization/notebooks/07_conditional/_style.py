"""Shared plot styling for the Gaussianization *conditional* notebooks.

Kept deliberately tiny so each notebook can ``from _style import style_ax,
GAUSS_KW`` and get a consistent look without re-deriving it. Mirrors
``notebooks/06_continuous_time/_style.py`` so the parts sit next to each other
without visual jarring.
"""
