# Code Adapted From: https://github.com/CIA-Oceanix/4dvarnet-core/blob/main/new_dataloading.py
...
class XrDataset(Dataset):
    ...
    def __init__(
        ...
        if resize_factor != 1:
            _ds = (
                _ds.coarsen(lon=resize_factor)
                .mean(skipna=True)
                .coarsen(lat=resize_factor)
                .mean(skipna=True)
            )
        ...
        dim_range_ = {
            "lon": slice(
                self.ds.lon.min().item() - dX[0], self.ds.lon.max().item() + dX[1]
            ),
            "lat": slice(
                self.ds.lat.min().item() - dY[0], self.ds.lat.max().item() + dY[1]
            ),
            "time": dim_range["time"],
        }
        ...
        self.ds = self.ds.pad(pad_, mode="reflect")
        ...
        self.ds_size = {
            dim: max(
                (self.ds.dims[dim] - slice_win[dim]) // self.strides.get(dim, 1) + 1, 0
            )
...
class FourDVarNetDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        oi_item = np.where(~np.isnan(_oi_item), _oi_item, 0.0)
        ...
        if self.sst_ds == None:
            ...
            sst_item = np.where(~np.isnan(_sst_item), _sst_item, 0.0)
...
class FourDVarNetDataModule(pl.LightningDataModule):
    def __init__(
        ...
        strides=None,
        train_slices=(
            slice("2012-10-01", "2012-11-20"),
            slice("2013-02-07", "2013-09-30"),
        ),
        ...
        self.train_slices, self.test_slices, self.val_slices = (
            train_slices,
            test_slices,
            val_slices,
        )
    ...
    def compute_norm_stats(self, ds):
        ...
        if self.sst_var == None:
            ...
            mean_sst = float(
                xr.concat(
                    [_ds.sst_ds.ds[_ds.sst_ds.var] for _ds in ds.datasets], dim="time"
                ).mean()
            )
            std_sst = float(
                xr.concat(
    ...
    def get_domain_bounds(self, ds):
        min_lon = round(
            np.min(np.concatenate([_ds.gt_ds.ds["lon"].values for _ds in ds.datasets])),
            2,
        )
        max_lon = round(
            np.max(np.concatenate([_ds.gt_ds.ds["lon"].values for _ds in ds.datasets])),
            2,
        )
        min_lat = round(
            np.min(np.concatenate([_ds.gt_ds.ds["lat"].values for _ds in ds.datasets])),
            2,
        )
        max_lat = round(
            np.max(np.concatenate([_ds.gt_ds.ds["lat"].values for _ds in ds.datasets])),
            2,
        )
