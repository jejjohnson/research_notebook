...
def bound_eval(output, target, mask):
    ...
    # batch_size = x.size(0)
    neg_elbo = nll_metric(x_recon, x, mask) + kl_div_metric(
        [mu_q, logvar_q], [mu_p, logvar_p], mask
    )
...
def importance_sample(
    batch_idx, model, x, x_reversed, x_seq_lengths, mask, n_sample=500
):
    ...
    for i in range(n_loop):
        ...
        ll_estimate_ = (
            log_px_z.sum(dim=1, keepdim=True)
    ...
    print(
        "%s-th batch, importance sampling took %.4f seconds."
